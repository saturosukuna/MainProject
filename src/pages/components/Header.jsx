...
const Header = ({ role, isConnected, connectWallet, disconnectWallet }) => {
  ...
  return (
    <header className="mt-1px-5 bg-gradient-to-r from-green-600 to-green-900 text-md shadow-md fixed top-0 left-0 right-0 z-50">
      <div className="container mx-auto flex justify-between items-center">
        {/* University Name */}
        <div className="flex items-center space-x-3">
          <img src="/images/au.png" className="logo w-20 p-2 h-15" />
        ...
        <nav className="hidden md:flex space-x-8 text-white text-lg">
          ...
          {isConnected && role === "admin" && (
            <NavLink to="/admin" className={({ isActive }) =>
              ...
              <span><iconify-icon icon="ic:round-admin-panel-settings" width="30px" noobserver></iconify-icon></span>
              <span>Admin</span>
            </NavLink>
          )}
          {isConnected && role === "staff" && (
            <NavLink to="/staff" className={({ isActive }) =>
              ...
              <span><iconify-icon icon="game-icons:teacher" width="30px" noobserver></iconify-icon></span>
              <span>Staff</span>
            </NavLink>
          )}
          {isConnected && role === "student" && (
            <NavLink to="/student" className={({ isActive }) =>
              ...
              <span><iconify-icon icon="hugeicons:students" width="30px" noobserver></iconify-icon></span>
              <span>Student</span>
            </NavLink>
          )}
    ...
    Connect Wallet
      ...
      {menuOpen && (
        <div className="md:hidden mt-4 bg-white rounded-lg shadow-lg text-center">
          ...
          {!isConnected ? (
            <button className="w-full bg-blue-600 px-4 py-2 hover:bg-blue-500 transition duration-300" onClick={connectWallet}>
              Connect Wallet
            </button>
          ) : (
            <button className="w-full bg-red-600 px-4 py-2 hover:bg-red-500 transition duration-300" onClick={disconnectWallet}>
              Disconnect Wallet
            </button>
          )}
        </div>
